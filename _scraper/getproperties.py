...
PROPERTY_EXTRACT_DATE = '2024/03/13'
RESULTS_PER_PAGE = 24
MIN_BEDS = 3
MAX_BEDS = 6
MAX_DAYS_ADDED = 1
...
async def run():
    ...
    for location in locations:
        ...
        if not os.path.exists(f'./properties/{PROPERTY_EXTRACT_DATE}/{loc}.json'):
            search_results = await scrape_search(location_ids[loc_array])
            json_object = json.dumps(search_results, indent=2)

            if not os.path.exists(f'./properties/{PROPERTY_EXTRACT_DATE}'):
                os.makedirs(f'./properties/{PROPERTY_EXTRACT_DATE}')

            with open(f"./properties/{PROPERTY_EXTRACT_DATE}/{loc}.json", "w") as outfile:
